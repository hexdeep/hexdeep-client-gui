...
export class CreateForm extends tsx.Component<IPorps, IEvents, ISlots> {
    ...
    // 镜像类型的"海外版(gms)"在安卓14下由独立的GMS开关取代，"松果版(pine)"安卓14下则完全没有对应镜像，
    // 二者都不再作为镜像类型选项展示；filterState.imageType 是安卓12/14两个tab共用的同一个observable，
    // 从安卓12切过来时如果还残留着"gms"/"pine"筛选，安卓14这边的单选框已经没有这一项可选中，需要一并重置为"base"。
    ...
    onAndroidVersionFilterChange(newVal: number) {
        ...
        if (newVal === 14 && (this.filterState.imageType === "gms" || this.filterState.imageType === "pine")) {
